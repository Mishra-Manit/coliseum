...
observability for the entire application including LLM calls.
...
class LogfireConfig:
    ...
    def initialize(cls, token: Optional[str] = None) -> None:
        ...
        Note:
            If token is missing, raises ValueError to ensure proper
        ...
        if not token:
            raise ValueError(
                "LOGFIRE_TOKEN environment variable or token argument must be provided."
            )

        logfire.configure(
            token=token,
            service_name="coliseum-api",
            send_to_logfire=True,
        )

        # Enable automatic instrumentation for pydantic-ai agents
        # This logs all LLM calls via OpenRouter with inputs, outputs,
