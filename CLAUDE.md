...
settings = get_settings()  # Singleton, loads .env + config.yaml
```
...
7. **Pipeline test always in paper mode**: After any backend pipeline change, validate by running `python -m coliseum pipeline`. **Never** set `trading.paper_mode: false` when testing — doing so places real-money orders on Kalshi. Confirm `paper_mode: true` in `backend/config.yaml` before every test run.
